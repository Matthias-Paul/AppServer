...
const ManageMediaInAService = () => {
  ...
  const removeMedia = async ({ mediaId }) => {
    const res = await fetch(`${baseURL}/api/services/${id}/${mediaId}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`
      }
  ...
  const handleRemove = (mediaId) => {
    if (window.confirm('Are you sure you want to remove this media from this service?')) {
    ...
    <>
      <div className="pt-6  text-[#0D47A1] w-full pl-7 overflow-hidden">
        <div className="flex justify-between items-center font-semibold text-[18px]  ">
          ...
          <Link to={`/admin/service/addExistingMedia/${serviceDetails?.id}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-end justify-end ">
                        <button
                          ...
                          className="px-4 py-[4px]  mt-4 mb-1  cursor-pointer text-[#E3F2FD] bg-[#0D47A1] font-semibold text-[18px] rounded-md "
                        >
                          Remove
                        </button>
                      </div>
...
export default ManageMediaInAService
