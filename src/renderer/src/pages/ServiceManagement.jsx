          ...
          <>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-5">
              {services.map((service) => {
                const imageURL =
                  service?.banner_image && baseURL ? `${baseURL}${service.banner_image}` : adminPic
                  ...
                  <div key={service?.id}>
                    <div className="flex flex-col items-start text-center justify-center border border-[#0D47A1] rounded-lg p-4">
                      <img
                        ...
                        src={imageURL}
                        alt={service?.name}
                        onError={(e) => (e.target.src = adminPic)}
                      />
                      <h1 className="font-bold text-start text-[25px] line-clamp-2 mt-5">
                        {service?.name}
                      </h1>
                      <h3 className="font-semibold text-start w-full truncate text-[18px]">Theme: {service?.theme}</h3>
