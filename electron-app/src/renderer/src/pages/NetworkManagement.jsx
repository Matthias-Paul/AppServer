...
const NetworkManagement = () => {
  ...
  const password = import.meta.env.VITE_NETWORK_PASSWORD || '********'
    ...
    <>
      <div className="pt-7 min-h-[400px] w-full pl-7 overflow-hidden ">
        ...
        <div className=" mt-8 grid grid-cols-2  gap-x-6 text-[#0D47A1]">
          <div className="border rounded-xl flex flex-col items-start px-9  justify-start border-[#0D47A1]   w-full ">
            ...
            <div className="flex items-center mt-1 font-semibold mb-4 w-full border border-[#E1E7F1] px-5 py-2 text-[20px] rounded justify-start">
            ...
            <div className="flex items-center border border-[#E1E7F1] w-full mb-6 px-4 py-2 rounded text-[20px]">
              <span className="flex-1">{show ? password : '•'.repeat(password.length)}</span>
              <span onClick={() => setShow(!show)} className="cursor-pointer ml-2">
                {show ? <HiEyeSlash /> : <HiEye />}
              </span>
            </div>
          </div>
          <div className="border rounded-xl px-12 flex pb-5 flex-col items-center justify-start border-[#0D47A1]  w-full ">
            <h2 className="mt-4  font-bold text-[25px] mb-5"> QR Code For Clients Connection </h2>
            <div className="border rounded-lg p-8 bg-white">
              <QRCode ref={qrRef} fgColor="#0D47A1" size={200} value={baseURL} />
            ...
            <div className="flex items-center gap-x-5 w-full justify-between">
              <button
                onClick={downloadQR}
                className=" active:text-[#0D47A1] active:bg-[#E3F2FD]  bg-[#0D47A1]  text-[#E3F2FD] text-xl lg:text-2xl cursor-pointer py-2 px-6 rounded-lg text-center"
              >
                Download QR
              </button>
              <button
